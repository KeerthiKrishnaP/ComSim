...
def NNA(Xc, Yc, Rc, IFD, rmax, Acc_fibers, L_RVE):
    ...
    temp_R = rmax
    temp_IFD = IFD
    # temporary fiber radius and IFD
    theta = 0
    # first search angle
    while fiber_found == 0:
...
def compatability_check(
    ...
    Pass = 0
    if (
        temp_X < -Tol or temp_Y < -Tol or temp_X >= L_RVE + Tol or temp_Y >= L_RVE + Tol
    ):  # the temp_X and temp_Y are in bounds (defines the shape of the packing)
        fiber_found = 0
    else:
